...
: Node("motion_test_server"), is_mode_check(false), patternCheckout_client_(NULL),
  ...
  heartbeat_err_cnt(0), heart_beat_thread_(nullptr), app_server_thread_(nullptr), dog_walk_thread_(nullptr), server_(nullptr),
  app_stub(nullptr), app_disconnected(true), destory_grpc_server_thread_(nullptr),
  change_mode_id(0), change_gait_id(0), ext_mon_id(0), dog_walk_(false)
{
  ...
  lpm_client_ = this->create_client<ception_msgs::srv::LpmCheck>(
    "request_lpm",
    rmw_qos_profile_services_default,
    callback_group_);
  ...
  led_client_ = this->create_client<ception_msgs::srv::SensorDetectionNode>("athena_led");
  timer_interval.init();
...
void Cyberdog_app::DogWalk()
{
  rclcpp::WallRate r(100ms);
  ...
  while(dog_walk_)
  {
    ...
    decissage_out.velocity.timestamp = get_clock()->now();
    decissage_out.velocity.linear_x = 0;
    decissage_out.velocity.linear_y = 0;
    decissage_out.velocity.linear_z = 1;
    decissage_out.velocity.angular_x = 0;
    decissage_out.velocity.angular_y = 0;
    decissage_out.velocity.angular_z = 0;
    decision_pub->publish(decissage_out);
    if(cnt++ == 10) {
      RCLCPP_INFO(get_logger(), "DogWalk");
      cnt = 0;
    }
    r.sleep();
  }
  ...
  ::grpc::ServerWriter<::cyberdogapp::CheckoutPattern_respond> * writer)
{
  ...
  while (true) {
    ...
    if (respond->get()->succeed() && request_.gait.gait == 12/*motion_msg::msg::Gait::DOG_WALK*/) {
      RCLCPP_INFO(get_logger(), "dog walk success");
      dog_walk_ = true;
      dog_walk_thread_ = std::make_shared<std::thread>(&Cyberdog_app::DogWalk, this);
      dog_walk_thread_->detach();
    }
...
void Cyberdog_app::subscribeActionResult(const motion_msgs::msg::ActionRespond::SharedPtr msg)
{
  if (msg->type == motion_msgs::msg::ActionRespond::CHECKOUT_MODE) {
    if (msg->request_id != change_mode_id) {
      RCLCPP_INFO(get_logger(), "get checkout mode result, invalid id");
      return;
    }
    ...
    if (msg->request_id != change_gait_id) {
      RCLCPP_INFO(get_logger(), "get checkout pattern result, invalid id");
      return;
    }
  ...
  ::cyberdogapp::TokenPass_Response * respond)
{
  ...
  ::cyberdogapp::Token respond_token;
  ::cyberdogapp::Header header;
  generate_grpc_Header(header, future_result.get()->info.header);
  respond_token.set_token(future_result.get()->info.token);
  respond_token.set_token_refresh(future_result.get()->info.token_refresh);
  respond_token.set_token_md5(future_result.get()->info.token_md5);
  respond_token.set_token_refresh_md5(future_result.get()->info.token_refresh_md5);
  respond_token.set_expire_in(future_result.get()->info.expire_in);
  respond_token.mutable_header()->CopyFrom(header);
  respond->mutable_info()->CopyFrom(respond_token);
}
  ...
  ::grpc::ServerWriter< ::cyberdogapp::ExtMonOrder_Respond> * writer)
{
  ...
  extMonOrderRespond_queue->clear();
  action_request_pub->publish(request_);
  while (true) {
...
void Cyberdog_app::setBtCmd(int command, std::string address)
{
  std::chrono::seconds timeout(5);
  auto request_ = std::make_shared<ception_msgs::srv::BtRemoteCommand::Request>();
...
bool Cyberdog_app::sendGoal(int audio_id)
{
  using namespace std::placeholders;

  if (!audio_client->wait_for_action_server(std::chrono::seconds(1))) {
    RCLCPP_INFO(get_logger(), "Audio server not available after waiting");
    return false;
  }
  ...
  RCLCPP_INFO(get_logger(), "Send audio play request");

  auto send_goal_options = rclcpp_action::Client<AudioPlayT>::SendGoalOptions();
  send_goal_options.goal_response_callback =
    std::bind(&Cyberdog_app::goal_response_callback, this, _1);
  send_goal_options.feedback_callback =
    std::bind(&Cyberdog_app::feedback_callback, this, _1, _2);
  send_goal_options.result_callback =
    std::bind(&Cyberdog_app::result_callback, this, _1);
  audio_client->async_send_goal(goal_msg, send_goal_options);
  return true;
}
void Cyberdog_app::goal_response_callback(std::shared_future<GoalHandleAudio::SharedPtr> future)
{
...
void Cyberdog_app::feedback_callback(
  GoalHandleAudio::SharedPtr,
  const std::shared_ptr<const AudioPlayT::Feedback> feedback)
{
  RCLCPP_INFO(get_logger(), "status: %u", feedback->feed.status);
}

void Cyberdog_app::result_callback(const GoalHandleAudio::WrappedResult & result)
{
  RCLCPP_INFO(get_logger(), "result: %u", result.result->result.error);
}
...
void Cyberdog_app::sendlpmEnable(
    const ::cyberdogapp::LpmStatus_Request * request,
    ::cyberdogapp::LpmStatus_Respond * respond)
{
  std::chrono::seconds timeout(15);
  auto request_ = std::make_shared<ception_msgs::srv::LpmCheck::Request>();
  request_->para = request->para();
  request_->command = request->command();
  if (!lpm_client_->wait_for_service()) {
    RCLCPP_INFO(get_logger(), "request lpm server not avalible");
    return;
  }
  auto future_result = lpm_client_->async_send_request(request_);
  std::future_status status = future_result.wait_for(timeout);
  if (status == std::future_status::ready) {
    RCLCPP_INFO(get_logger(), "Succeed to send lpm command.");
  } else {
    RCLCPP_INFO(get_logger(), "Failed to send lpm command.");
  }
  respond->set_command(request->command());
  respond->set_result(future_result.get()->result);
}
