...
::grpc::Status CyberdogAppImpl::sendLpmEnable(
  ::grpc::ServerContext * context,
  const ::cyberdogapp::LpmStatus_Request * request,
  ::grpc::ServerWriter<::cyberdogapp::LpmStatus_Respond> * writer)
{
  ::cyberdogapp::LpmStatus_Respond respond;
  if (isPeerAvalible(context->peer())) {
    decision_->sendlpmEnable(request, &respond);
    writer->Write(respond);
  }
  return Status::OK;
}
...
void CyberdogAppImpl::process_motion(const ::cyberdogapp::MotionCommand * request)
{
  switch (request->command()) {
    case ::cyberdogapp::MotionCommand::TEST_INIT:
      ...
      system("/opt/ros2/foxy/bin/ros2 run athena_motion_test motion_test_server");
      break;
