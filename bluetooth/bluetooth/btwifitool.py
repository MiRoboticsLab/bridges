...
def deleteNet(ssid):
    ...
    not_found = 'cannot delete unknown'
    delcmd = "sudo nmcli con delete '"
    delcmd += ssid
    delcmd += "'"
    output = subprocess.Popen(delcmd,
                              shell=True,
                              stdout=subprocess.PIPE,
                              stderr=subprocess.STDOUT)
    output.wait()
    result = str(output.stdout.read(), encoding='utf-8')
    if operator.contains(result, del_success) or \
            operator.contains(result, not_found):
        return True
    else:
        return False
