...
class RemoteCommandService(Node):
    # for bt remote control
    GET_STATUS = 0x00
    SCAN_DEVICE = 0x01
    CONNECT_DEVICE = 0x02
    DISCONNECT_DEVICE = 0x03
    REMOTE_RECONNECT_DEVICE = 0x04

    # for bt adapter
    GET_BT_ADV_ATT_STATE = 0x10
    SET_BT_ADV_ATT_ENABLE = 0x11
    SET_BT_ADV_ATT_DISABLE = 0x12
    ...
    def command_callback(self, request, response):
        ...
        response.success, t = True, None

        # for bt remote control
        if request.command == self.GET_STATUS:
            t = Thread(target=getAllStatus)
        elif request.command == self.SCAN_DEVICE:
            if gattserver.bleStatus is gattserver.BLE_STATUS_NONE or \
                    gattserver.bleStatus is gattserver.BLE_STATUS_DISCONNECTED:
        ...
        elif request.command == self.CONNECT_DEVICE:
            if gattserver.bleStatus is gattserver.BLE_STATUS_NONE or \
                    gattserver.bleStatus is gattserver.BLE_STATUS_DISCONNECTED:
                if gattserver.enableAutoRemoteControl is True:
                    stopAutoConnect()
        ...
        elif request.command == self.DISCONNECT_DEVICE:
            if gattserver.bleStatus is gattserver.BLE_STATUS_NONE or \
                    gattserver.bleStatus is gattserver.BLE_STATUS_DISCONNECTED:
                if gattserver.enableAutoRemoteControl is True:
                    stopAutoConnect()
        ...
        elif request.command == self.REMOTE_RECONNECT_DEVICE:
            if gattserver.bleStatus is gattserver.BLE_STATUS_NONE or \
                    gattserver.bleStatus is gattserver.BLE_STATUS_DISCONNECTED:
                if gattserver.enableAutoRemoteControl is True:
                    stopAutoConnect()
        ...
        # for bt adapter
        elif request.command == self.GET_BT_ADV_ATT_STATE:
            response.success = False
            pass
        elif request.command == self.SET_BT_ADV_ATT_ENABLE:
            gattserver.SetBT_ADVATT_Enable(True)
        elif request.command == self.SET_BT_ADV_ATT_DISABLE:
            gattserver.SetBT_ADVATT_Enable(False)
        # for future command
        else:
            response.success = False
