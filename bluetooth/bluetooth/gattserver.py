...
class RobotAdvertisement(Advertisement):
    ...
    def __init__(self, bus, index):
        ...
        f_psn = subprocess.Popen('seek_cat /dev/mmcblk0p11 10485760  | sed -n \'2p\' | cut -c 6- | sed s/.$//',
...
def handleThread():
    ...
    wifiConnection = True  # connection start
    # disconnect wlan, or prevent wlan reconnect
    output = btwifitool.disWifi()
    log.logger.info('disWifi :' + repr(output))
    if not btwifitool.getDisStatus(output):
        log.logger.debug('disconnect current wifi connect failed!!!!!')
        pass
    if not btwifitool.deleteNet(mainSSID):  # del ssid
        log.logger.warning('del wifi ssid failed')
    time.sleep(1)  # sleep for wlan scan

    timer = TimeoutTimer(20)
    timer.startTimer()
    while not timer.getAlarm():
        output = btwifitool.nmcliConnectWifi(mainSSID, mainPwd)  # do a new connection
        log.logger.info('handleThread nmcliConnectWifi result:' + repr(output))
        tmpStatus = btwifitool.return_connect_status(output)  # check connection status
        if tmpStatus == STATUS_WIFI_SUCCESS:
            timer.stopTimer()
    ...
    def listener_callback(self, msg):
        ...
        if BLE_STATUS_NONE == bleStatus:
            ...
            if enableReTouchADV is True:
                ...
                log.logger.debug('ble adv is already started')
...
def SetBT_ADVATT_Enable(state):
    global log, bleStatus, audioClient, ledClient
    if state is True:
        if bleStatus is BLE_STATUS_NONE or \
                bleStatus is BLE_STATUS_DISCONNECTED:
            bleStatus = BLE_STATUS_ADV
            RegADV(True)
            RegATT(True)
            if audioClient.check_ready():
                log.logger.debug('enable ADV and ATT, audio send TOUCH_ADV_OR_WIFI_CONNECT_SUCCESS')
                audioClient.send_goal(audioClient.TOUCH_ADV_OR_WIFI_CONNECT_SUCCESS)
            if ledClient.check_ready():
                log.logger.debug('enable ADV and ATT, led send LED_BT_ADV')
                ledClient.send_request(ledClient.LED_BT_ADV,
                                       ledClient.TYPE_FUNCTION,
                                       5000000000)
    else:
        if bleStatus is BLE_STATUS_ADV:
            bleStatus = BLE_STATUS_DISCONNECTED  # not none
            RegADV(False)
            RegATT(False)
            if ledClient.check_ready():
                if ledClient.get_last_always():
                    log.logger.debug('disable ADV and ATT, led send LED_BT_ADV OFF')
                    ledClient.send_request(ledClient.LED_BT_ADV,
                                           ledClient.TYPE_FUNCTION,
                                           ledClient.COMMAND_OFF)
